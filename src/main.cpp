...
void drawLocationIcon(int x, int y);
...
{
    // Get time in miliseconds since boot
    StartTime = millis();
...
void setup()
{
    ...
    else
    {
        ...
        // setup i2c
        bool sensorIO = setupI2C();

        if (sensorIO)
        {
            startInternalMeasurement();
        }
        else
        {
            Serial.println("Sensor config failed");
        }
        ...
        Language   = json1["OpenWeather"]["language"].as<String>();
        ...
        if (StartWiFi() == WL_CONNECTED && SetupTime() == true)
        {
            ...
            // {
                ...
                {                       // Only if received both Weather or Forecast proceed
                    ...
                    getInternalData();  // Read the Temperature/Humidity sensor
...
bool setupI2C()
{
    ...
    i2c_config.scl_io_num = I2C_MASTER_SCL;
    ...
    if (!i2c_driver_install(I2C_MASTER_NUM, i2c_config.mode, I2C_MASTER_RX_BUF_DISABLE, I2C_MASTER_TX_BUF_DISABLE, 0) == ESP_OK)
    {
        setupIo = false;
        Serial.println("Sensor install failed");
    }
...
void startInternalMeasurement()
{
    Serial.println("Starting internal measurement");
...
void getInternalData()
{
    ...
    WxConditions[0].HumidityIndoor = sensors_values.humidity;

    Serial.print("Internal Temperature: ");
    Serial.println(sensors_values.temperature);

    Serial.print("Internal Humidity: ");
    Serial.println(sensors_values.humidity);   
...
bool DecodeWeather(WiFiClient &json, String Type)
{
    ...
    if (Type == "weather")
    {
        ...
        WxConditions[0].TemperatureOutdoor = root["main"]["temp"].as<float>();
        Serial.println("Temp: " + String(WxConditions[0].TemperatureOutdoor));
        WxConditions[0].Pressure = root["main"]["pressure"].as<float>();
        Serial.println("Pres: " + String(WxConditions[0].Pressure));
        WxConditions[0].HumidityOutdoor = root["main"]["humidity"].as<float>();
        Serial.println("Humi: " + String(WxConditions[0].HumidityOutdoor));
    ...
    if (Type == "forecast")
    {
        ...
        for (byte r = 0; r < max_readings; r++)
        {
            ...
            WxForecast[r].TemperatureOutdoor = list[r]["main"]["temp"].as<float>();
            Serial.println("Temp: " + String(WxForecast[r].TemperatureOutdoor));
            ...
            WxForecast[r].HumidityOutdoor = list[r]["main"]["humidity"].as<float>();
            Serial.println("Humi: " + String(WxForecast[r].HumidityOutdoor));
...
bool obtainWeatherData(WiFiClient &client, const String &RequestType)
{
    ...
    String uri = "http://api.openweathermap.org/data/2.5/" + RequestType + "?q=" + City + "," + Country + "&APPID=" + apikey + "&mode=json&units=" + units + "&lang=" + Language;

    //String uri = "http://api.openweathermap.org/data/2.5/weather?q=Wil,CH&APPID=" + apikey + "&units=metric" + "&lang=" + Language;
    ...
    Serial.printf("HTTP Code: %s", http.errorToString(httpCode).c_str());
...
{    
    ...
    // Astronomy section Sun rise/set, Moon phase and Moon icon
    DisplayAstronomySection(5, 255);

    // Centre section of display for Location, temperature, Weather report, current Wx Symbol 
    DisplayMainWeatherSection(320, 110);

    // Display weather icon    scale = Large;
    DisplayWeatherIcon(820, 130);

    // 3hr forecast boxes
    DisplayForecastSection(325, 200);

}

void DisplayGeneralInfoSection()
{
    drawLocationIcon(12, 2);
    setFont(OpenSans10B);

    if(City == "Wil")
    {
        drawString(22, 2, "Wil SG", LEFT);
    }
    else
    {
        drawString(20, 2, City, LEFT); // 5,2
    }
    
    setFont(OpenSans8B);
    drawString(250, 2, "Letztes Update: " + Date_str + "  @   " + Time_str, LEFT);
...
void DisplayMainWeatherSection(int x, int y)
{
    ...
    DisplayTemperatureSectionIndoor(x + 210, y - 40); // for displaying the indoor temperature
    DisplayPressureSection(x - 25, y + 70, WxConditions[0].Pressure, WxConditions[0].Trend);
}
...
{
    setFont(OpenSans10B);
    drawString(x - 50, y - 30, "Aussen", LEFT);    
    setFont(OpenSans18B);
    drawString(x - 50, y, String(WxConditions[0].TemperatureOutdoor, 1) + "°    " + String(WxConditions[0].HumidityOutdoor, 0) + "%", LEFT);
    setFont(OpenSans12B);
    drawString(x + 50, y + 35, String(WxConditions[0].High, 0) + "° max | " + String(WxConditions[0].Low, 0) + "° min", CENTER); // Show forecast high and Low
}
...
#define lineWidth 34
    ...
    drawString(x + 10, y + 5, TitleCase(Line1), LEFT);
    if (Line1 != Line2)
        drawString(x + 40, y + 30, Line2, LEFT);
}
...
String formatNumber(int number) {
    if (number >= 1000) {
        // Extract thousands and remainder
        int thousands = number / 1000;
        int remainder = number % 1000;

        // Convert to string
        String result = String(thousands) + "'";

        // Add remainder with manual zero-padding
        if (remainder < 100) result += "0";
...
void DisplayPressureSection(int x, int y, float pressure, String slope)
{
    ...
    {
        Visibility(x + 145, y, formatNumber(WxConditions[0].Visibility) + " Meter");
        x += 150; // Draw the text in the same positions if one is zero, otherwise in-line
    }
    if (WxConditions[0].Cloudcover > 0)
        CloudCover(x + 220, y, WxConditions[0].Cloudcover);
}
...
void DisplayForecastWeather(int x, int y, int index)
{
    ...
    drawString(x + fwidth / 2, y + 150, String(WxForecast[index].HumidityOutdoor, 0) + "%", CENTER);
}

void DisplayAstronomySection(int x, int y)
{
    setFont(OpenSans10B);
    drawString(x + 5, y + 20, "Dämmerung | Mondphase", LEFT);
    drawString(x + 5, y + 45, ConvertUnixTime(WxConditions[0].Sunrise).substring(0, 5) + " " + TXT_SUNRISE, LEFT);
    drawString(x + 5, y + 70, ConvertUnixTime(WxConditions[0].Sunset).substring(0, 5) + " " + TXT_SUNSET, LEFT);
    ...
    setFont(OpenSans8B);
    String phase = MoonPhase(day_utc, month_utc, year_utc, Hemisphere);

    if (phase.length() > 22)
    {   
        int space = phase.indexOf(" ");
        String s1 = phase.substring(0, space);
        String s2 = phase.substring(space + 1, phase.length());

        drawString(x + 5, y + 95, s1, LEFT);
        drawString(x + 5, y + 110, s2, LEFT);

    } else {
        drawString(x + 5, y + 95, phase, LEFT);

    }

    DrawMoon(x + 200, y + 10, day_utc, month_utc, year_utc, Hemisphere);
...
void DisplayForecastSection(int x, int y)
{
    ...
    { // Pre-load temporary arrays with with data - because C parses by reference and remember that[1] has already been converted to I units
        ...
        temperature_readings[r] = WxForecast[r].TemperatureOutdoor;
        humidity_readings[r] = WxForecast[r].HumidityOutdoor;
        r++;
...
void DrawPressureAndTrend(int x, int y, float pressure, String slope)
{
    ...
    drawString(x + 25, y - 10, formatNumber(pressure) + (Units == "M" ? " hPa" : "in"), LEFT);
    if (slope == "+")
    {
...
#define number_of_dashes 20
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        drawString(20 + x_pos + gwidth / 3 * i + 27, y_pos + gheight + 10, String(i * 24 + 24), LEFT);
        if (i < 2)
...
{
    int32_t radius = 5;
    int32_t radiusSmall = 2;
    epd_fill_circle(x, (y + 10)/2, radius, Black, framebuffer);
    epd_fill_triangle(x - radius, (y + 10)/2, x + radius, (y + 10)/2, x , y + 15, Black, framebuffer);
    epd_fill_circle(x, (y + 10)/2, radiusSmall, White, framebuffer);
    epd_draw_oval(x, y + 15, 5, 2, Black, framebuffer);
