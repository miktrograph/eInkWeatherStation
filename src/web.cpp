...
void setupWEB(void)
{
    ...
    //File file = SPIFFS.open("/webView.html", FILE_WRITE, true);
    
    //if (file.print(webPage) == 0)
    //{
    //    Serial.println(F("Failed to write the HTML file"));
    //
    //} else {
    //    Serial.println("HTML File write successful");
    //
    //}
    ...
    server.on("/config", HTTP_GET, []() {
        ...
        //File file = SPIFFS.open("/webView.html", "r");
...
void storeConfig()
{
    ...
    for ( uint8_t i = 0; i < server.args(); i++ )
    {
        ...
        else if (server.argName(i).equals("language"))
        {
            doc["OpenWeather"]["language"] = server.arg(i);
        }
